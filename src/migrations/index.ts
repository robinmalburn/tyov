  ...
  async migrate(data, signature) {
    ...
    migrations.forEach((migration) => {
      if (!migration.requiredSignature) {
        console.warn(
          `Skipping migration with missing signature.  Description: ${migration.description}`,
        )
        return
      }
...
migrator.register(
  async (signature) =>
    signature < 1 && import('Migrations/20210414-addSkillsIdMigration'),
  async (signature) =>
    signature < 1 &&
    import('Migrations/20210415-addMarkDescriptionIdMigration'),
  async (signature) =>
    signature < 1 && import('Migrations/20210415-addResourcesIdMigration'),
  async (signature) =>
    signature < 1 && import('Migrations/20210415-addCharactersIdMigration'),
  async (signature) =>
    signature < 1 &&
    import('Migrations/20210415-addMemoryIdEventDescriptionMigration'),
  async (signature) =>
    signature < 1 && import('Migrations/20210416-addPromptIdMigration'),
  async (signature) =>
    signature < 2 &&
    import('Migrations/20210501-decoupleMemoryEventDiaryStates'),
)
